...
(function(_oG) {
/*! Ldc 0.0.7 //// MIT Licence //// http://ldc.richplastow.com/ */
...
Item = (function() {
  ...
  function Item(config) {
    ...
    this.id = v('id <string ^[a-z]\\w{1,23}$>');
...
Ldc = (function() {
  ...
  Ldc.prototype.addItem = function(kind, config, id) {
    var M, kinds, v;
    ...
    if (!_o.isU(this.items[id])) {
      throw RangeError(M + ("an Item with id '" + id + "' already exists"));
    }
    config.id = id;
    config.prev = this.items._length ? this.items._last : null;
    config.next = null;
    this.items[id] = new kinds[kind](config);
    if (this.items._length) {
      this.items._last.next = this.items[id];
    } else {
      this.items._first = this.items[id];
    }
    this.items._last = this.items[id];
    this.items._length++;
    return id;
  };

  Ldc.prototype.deleteItem = function(id) {
    var M, item, v;
    ...
    if (_o.isU(this.items[id])) {
      throw RangeError(M + ("the Item with id '" + id + "' does not exist"));
    }
    item = this.items[id];
    if (item.prev) {
...
tudor.add([
  "01 Ldc Constructor", tudor.is, "The class and instance are expected types", "The Ldc class is a function", _o.F, function() {
    ...
    return new Ldc;
  }, "Instance properties as expected", "`Ldc::items` is an object", _o.O, function() {
    return (new Ldc).items;
  }, tudor.equal, "`Ldc::items` is empty", 0, function() {
    return (new Ldc).items._length;
  }, "`Ldc::items` has no `_first` Item", null, function() {
    return (new Ldc).items._first;
  }, "`Ldc::items` has no `_last` Item", null, function() {
    return (new Ldc).items._last;
  }
]);
...
tudor.add([
  "02 Ldc::addItem()", tudor.is, "`addItem()` is a function which returns a string", function() {
    ...
    return ldc.addItem;
  }, "`addItem('Hat',{},'the_first')` returns a string", _o.S, function(ldc) {
    return ldc.addItem('Hat', {}, 'the_first');
  ...
  }, "The `id` argument accepts a string as expected", tudor.equal, "Shortest possible id", 7, function(ldc) {
    ldc.addItem('Hat', void 0, 'aB');
    return ldc.items._length;
  }, "Longest possible id", 'abcdefghijklmnopqrst123_', function(ldc) {
    return ldc.addItem('Hat', {}, 'abcdefghijklmnopqrst123_');
  }, "Can repeat existing id, if case is different", 9, function(ldc) {
    ldc.addItem('Hat', {}, 'aBcDeFgHiJkLmNoPqRsT123_');
    return ldc.items._length;
  }, "`id` exceptions", tudor["throw"], "Is boolean", "/ldc/src/Ldc.litcoffee Ldc::addItem()\n  argument id is type boolean not string", function(ldc) {
    return ldc.addItem('Hat', {}, true);
  }, "Empty string", "/ldc/src/Ldc.litcoffee Ldc::addItem()\n  argument id fails ^[a-z]\\w{1,23}$", function(ldc) {
  ...
  }, "Must be unique", "/ldc/src/Ldc.litcoffee Ldc::addItem()\n  an Item with id 'the_last' already exists", function(ldc) {
    ldc.addItem('Hat', {}, 'the_last');
    return ldc.addItem('Sine', {}, 'the_last');
  }, "`_first`, `_last`, `prev` and `next` as expected", tudor.equal, "The first Item is 'the_first'", 'the_first', function(ldc) {
    return ldc.items._first.id;
  }, "The last Item is 'the_last'", 'the_last', function(ldc) {
    return ldc.items._last.id;
  }, "The first Item’s `prev` is null", null, function(ldc) {
    return ldc.items._first.prev;
  }, "The last Item’s `next` is null", null, function(ldc) {
    return ldc.items._last.next;
  }, "The first Item’s `next` is 'the_second'", 'the_second', function(ldc) {
    return ldc.items._first.next.id;
  }, "The last Item’s `prev` is 'aBcDeFgHiJkLmNoPqRsT123_'", 'aBcDeFgHiJkLmNoPqRsT123_', function(ldc) {
    return ldc.items._last.prev.id;
  }
]);
...
tudor.add([
  ...
  }, "Instance properties as expected", function() {
    return [
      new Item({
        id: 'test_item',
        prev: null,
  ...
  }, "`Item::id` is a string", _o.S, function() {
    return (new Item({
      id: 'abc',
      prev: null,
      next: null
    })).id;
  }, "`Item::prev` can be null", _o.X, function() {
    return (new Item({
      id: 'abc',
      prev: null,
      next: null
    })).prev;
  }, "`Item::prev` can be another Item", _o.O, function(item, tone) {
    return (new Item({
      id: 'abc',
      prev: item,
      next: null
    })).prev;
  }, "`Item::next` can be null", _o.X, function() {
    return (new Item({
      id: 'abc',
      prev: null,
      next: null
    })).next;
  }, "`Item::next` can be another Item", _o.O, function(item, tone) {
    return (new Item({
      id: 'abc',
      prev: null,
      next: tone
    })).next;
  }, tudor.equal, "`Item::id` is as passed-in", 'abc', function() {
    return (new Item({
      id: 'abc',
      prev: null,
      next: null
    })).id;
  }, "`Item::prev` is as passed-in", 'test_item', function(item, tone) {
    return (new Item({
      id: 'abc',
      prev: item,
      next: null
    })).prev.id;
  }, "`Item::next` is as passed-in", 'test_tone', function(item, tone) {
    return (new Item({
      id: 'abc',
      prev: null,
      next: tone
    })).next.id;
  }, "`config.id` exceptions", tudor["throw"], "No `config.id`", "/ldc/src/Item.litcoffee Item()\n  config.id is undefined and has no fallback", function() {
    return new Item({
      prev: null,
      next: null
    });
  }, "Is boolean", "/ldc/src/Item.litcoffee Item()\n  config.id is type boolean not string", function() {
    return new Item({
      id: true,
      prev: null,
      next: null
    });
  }, "Empty string", "/ldc/src/Item.litcoffee Item()\n  config.id fails ^[a-z]\\w{1,23}$", function() {
    return new Item({
      id: '',
      prev: null,
      next: null
  ...
  }, "`config.prev` exceptions", "No `config.prev`", "/ldc/src/Item.litcoffee Item()\n  config.prev is undefined and has no fallback", function() {
    return new Item({
      id: 'abc'
    });
  }, "Is string", "/ldc/src/Item.litcoffee Item()\n  config.prev is type string not Item|null", function() {
    return new Item({
      id: 'abc',
      prev: '3'
    });
  }, "Is a number", "/ldc/src/Item.litcoffee Item()\n  config.prev is type number not Item|null", function() {
    return new Item({
      id: 'abc',
      prev: 3.5
    });
  }, "Is a generic object", "/ldc/src/Item.litcoffee Item()\n  config.prev is type object not Item|null", function() {
    return new Item({
      id: 'abc',
      prev: {}
    });
  }, "Does not inherit from Item", "/ldc/src/Item.litcoffee Item()\n  config.prev is type object not Item|null", function() {
    return new Item({
      id: 'abc',
      prev: new Ldc
    });
...
tudor.add([
  "04 Ldc::deleteItem()", tudor.is, "`deleteItem()` is a function which returns `undefined`", function() {
    return [new Ldc];
  }, "`deleteItem()` is a function", _o.F, function(ldc) {
    return ldc.deleteItem;
  }, "deleting an Item returns `undefined`", _o.U, function(ldc) {
    ldc.addItem('Hat', {}, 'test');
    return ldc.deleteItem('test');
  }, "The `id` argument accepts a string as expected", tudor.equal, "Shortest possible id", 0, function(ldc) {
    ldc.addItem('Hat', {}, 'aB');
    ldc.deleteItem('aB');
    return ldc.items._length;
  }, "Longest possible id", void 0, function(ldc) {
    ldc.addItem('Hat', {}, 'abcdefghijklmnopqrst123_');
    ldc.deleteItem('abcdefghijklmnopqrst123_');
    return ldc.items.abcdefghijklmnopqrst123_;
  }, "`id` exceptions", tudor["throw"], "Is an array", "/ldc/src/Ldc.litcoffee Ldc::deleteItem()\n  argument id is type array not string", function(ldc) {
    ldc.addItem('Hat', {}, 'abc123');
    return ldc.deleteItem(['abc123']);
  }, "Empty string", "/ldc/src/Ldc.litcoffee Ldc::deleteItem()\n  argument id fails ^[a-z]\\w{1,23}$", function(ldc) {
    return ldc.deleteItem('');
  }, "Too short", "/ldc/src/Ldc.litcoffee Ldc::deleteItem()\n  argument id fails ^[a-z]\\w{1,23}$", function(ldc) {
    return ldc.deleteItem('a');
  }, "Too long", "/ldc/src/Ldc.litcoffee Ldc::deleteItem()\n  argument id fails ^[a-z]\\w{1,23}$", function(ldc) {
    return ldc.deleteItem('aBcDeFgHiJkLmNoPqRsT123_X');
  }, "Underscore is an invalid first character", "/ldc/src/Ldc.litcoffee Ldc::deleteItem()\n  argument id fails ^[a-z]\\w{1,23}$", function(ldc) {
    return ldc.deleteItem('_abc');
  }, "Number is an invalid first character", "/ldc/src/Ldc.litcoffee Ldc::deleteItem()\n  argument id fails ^[a-z]\\w{1,23}$", function(ldc) {
    return ldc.deleteItem('1abc');
  }, "Uppercase is an invalid first character", "/ldc/src/Ldc.litcoffee Ldc::deleteItem()\n  argument id fails ^[a-z]\\w{1,23}$", function(ldc) {
    return ldc.deleteItem('Abc');
  }, "Must not contain a hyphen", "/ldc/src/Ldc.litcoffee Ldc::deleteItem()\n  argument id fails ^[a-z]\\w{1,23}$", function(ldc) {
    return ldc.deleteItem('ab-c');
  }, "Must exist in `items`", "/ldc/src/Ldc.litcoffee Ldc::deleteItem()\n  the Item with id 'nope' does not exist", function(ldc) {
    return ldc.deleteItem('nope');
  }, "`_first`, `_last`, `prev` and `next` as expected", tudor.equal, "After deleting 'abc123', `items._length` is 0...", 0, function(ldc) {
    ldc.deleteItem('abc123');
    return ldc.items._length;
  }, "...`items._first` is null...", null, function(ldc) {
    return ldc.items._first;
  }, "...`items._last` is null...", null, function(ldc) {
    return ldc.items._last;
  }, "...and `items` stringifies as expected", '{"_length":0,"_first":null,"_last":null}', function(ldc) {
    return JSON.stringify(ldc.items);
  }, "After adding three Items and deleting the middle, `items._length` is 2", 2, function(ldc) {
    ldc.addItem('Hat', {}, 'the_first');
    ldc.addItem('Hat', {}, 'the_second');
    ldc.addItem('Sine', {}, 'the_third');
    ldc.deleteItem('the_second');
    return ldc.items._length;
  }, "...`items._first` is 'the_first'...", 'the_first', function(ldc) {
    return ldc.items._first.id;
  }, "...`items._last` is 'the_third'...", 'the_third', function(ldc) {
    return ldc.items._last.id;
  }, "...the first Item’s `next` is 'the_third'...", 'the_third', function(ldc) {
    return ldc.items._first.next.id;
  }, "...and the last Item’s `prev` is 'the_first'", 'the_first', function(ldc) {
    return ldc.items._last.prev.id;
  }, "After deleting the first Item, the last Item’s `prev` is null...", null, function(ldc) {
    ldc.deleteItem('the_first');
    return ldc.items._last.prev;
  }, "...and `items._first` is 'the_third'", 'the_third', function(ldc) {
    return ldc.items._first.id;
  }
]);
}).call(this,this);
